...
class StoreRepository implements StoreRepositoryInterface {
  ...
  // =========================
  // Header & Module utilities
  // =========================

  int? _currentModuleId() {
    final splash = Get.find<SplashController>();

    // 1) إذا موجود بالميموري
    if (splash.module?.id != null) return splash.module!.id!;

    // 2) fallback من الكاش
    final cached = sharedPreferences.getString(AppConstants.cacheModuleId);
    if (cached != null) {
      try {
        return ModuleModel.fromJson(jsonDecode(cached)).id;
      } catch (_) {}
    }
    return null;
  }

  Map<String, String> _buildHeader({int? moduleId}) {
    final AddressModel? addressModel = AddressHelper.getUserAddressFromSharedPref();
    final String? languageCode = sharedPreferences.getString(AppConstants.languageCode);

    // مهم: setHeader:false عشان ما نلوّث الهيدر العام لباقي الأقسام
    return apiClient.updateHeader(
    ...
    switch (source) {
      case DataSourceEnum.client:
        ...
        if (response.statusCode == 200) {
          ...
          LocalClient.organize(DataSourceEnum.client, cacheId, jsonEncode(response.body), header);
        }
        break;
  ...
  Future<List<Store>?> _getPopularStoreList(String type, {required DataSourceEnum source}) async {
    ...
    final String cacheId = '${AppConstants.popularStoreUri}?type=$type-moduleId=${moduleId ?? 'null'}';
  ...
  Future<List<Store>?> _getLatestStoreList(String type, {required DataSourceEnum source}) async {
    ...
    final String cacheId = '${AppConstants.latestStoreUri}?type=$type-moduleId=${moduleId ?? 'null'}';
  ...
  Future<List<Store>?> _getTopOfferStoreList({required DataSourceEnum source, String? filterBy, String? sortBy}) async {
    ...
    final String computedFilter = (filterBy == '1')
        ? 'halal=1'
        : (filterBy == 'veg')
            ? 'type=veg'
            : (filterBy == 'non_veg')
    ...
    switch (source) {
      case DataSourceEnum.client:
        final Response response = await apiClient.getData(
          '${AppConstants.topOfferStoreUri}?sort_by=$sortBy&$computedFilter',
          headers: header,
        );
        if (response.statusCode == 200) {
          topOfferStoreList = [];
  ...
  Future<List<Store>?> _getFeaturedStoreList({required DataSourceEnum source}) async {
    ...
    final String cacheId = '${AppConstants.storeUri}/all?featured=1&offset=1&limit=50-moduleId=${moduleId ?? 'null'}';
  ...
  Future<List<Store>?> _getVisitAgainStoreList({required DataSourceEnum source}) async {
    ...
    final String cacheId = '${AppConstants.visitAgainStoreUri}-moduleId=${moduleId ?? 'null'}';

    switch (source) {
      case DataSourceEnum.client:
        final Response response = await apiClient.getData(AppConstants.visitAgainStoreUri, headers: header);
        if (response.statusCode == 200) {
          visitAgainStoreList = [];
  ...
  Future<List<Store>?> _getRecommendedStoreList({required DataSourceEnum source}) async {
    ...
    final String cacheId = '${AppConstants.recommendedStoreUri}-moduleId=${moduleId ?? 'null'}';

    switch (source) {
      case DataSourceEnum.client:
        Response response = await apiClient.getData(AppConstants.recommendedStoreUri, headers: header);
        if (response.statusCode == 200) {

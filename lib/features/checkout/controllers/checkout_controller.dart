...
class CheckoutController extends GetxController implements GetxService {
  ...
  if (_store != null) {
    // أول شيء نجيب الـ surge price (لو الـ API متوفرة)
    await getSurgePrice(
  ...
  try {
    final SurgePriceModel? surgePriceModel =
        await checkoutServiceInterface.getSurgePrice(
      zoneId: zoneId,
      moduleId: moduleId,
      dateTime: dateTime,
      guestId: guestId,
    ...
    _surgePrice = surgePriceModel; // لو null يعني ما في Surge حالياً

    if (kDebugMode) {
      print('✅ Surge price loaded: $_surgePrice');
    }
  } catch (e) {
    // أي خطأ هنا نتجاهله ونكمّل عادي
    if (kDebugMode) {
      print('⚠️ Surge price error (ignored): $e');
    }
    _surgePrice = null;
  }

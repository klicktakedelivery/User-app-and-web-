...
class CheckoutScreenState extends State<CheckoutScreen> {
  ...
  Future<void> initCall() async {
    ...
    Get.find<CheckoutController>().streetNumberController.text = userAddress?.streetNumber ?? '';
    Get.find<CheckoutController>().houseController.text = userAddress?.house ?? '';
    Get.find<CheckoutController>().floorController.text = userAddress?.floor ?? '';
    ...
    if ((!_firstTimeCheckPayment &&
      ...
      Future.delayed(const Duration(milliseconds: 600), () {
        ...
        Get.find<CheckoutController>().changeDigitalPaymentName(
          Get.find<SplashController>().configModel!.activePaymentMethodList![0].getWay!,
          willUpdate: false,
        );
        ...
        return GetBuilder<CouponController>(builder: (couponController) {
          ...
          Future.delayed(const Duration(milliseconds: 50), () {
            if (checkoutController.isFirstTime ||
                (couponController.discount! > 0 && !checkoutController.isFirstTime && !_calledOrderTax)) {
                ...
                for (int index = 0; index < _cartList!.length; index++) {
                        ...
                        j++) {
                          if (cart.foodVariations![i][j]!) {
                            variations[variations.length - 1]
                                .values!
                                .label!
                                .add(cart.item!.foodVariations![i].variationValues![j].level);
                          }
                        }
                      }
                    }
                  ...
                  carts.add(
                    OnlineCart(
                      ...
                      '',
                      Get.find<SplashController>().getModuleConfig(cart.item!.moduleType).newVariation!
                          ? null
                          : cart.variation,
                      Get.find<SplashController>().getModuleConfig(cart.item!.moduleType).newVariation!
                          ? variations
          ...
          double additionalCharge = Get.find<SplashController>().configModel!.additionalChargeStatus! &&
              Get.find<SplashController>().configModel!.additionCharge != null
          ...
          if (checkoutController.orderType != 'take_away' && checkoutController.store != null) {
            _deliveryChargeForView = (checkoutController.orderType == 'delivery'
          ...
          widget.storeId != null ? 0 : _calculateExtraPackagingCharge(checkoutController);
          ...
          checkoutController.setTotalAmount(
            total - (checkoutController.isPartialPay ? walletBalance : 0),
          );
          ...
          onPressed: checkoutController.acceptTerms
            ...
            if (isGuestLogIn &&
              ...
              showCustomSnackBar(
                ...
                widget.storeId != null) {
              showCustomSnackBar(
                  'payment_method_is_not_available'.tr);
              ...
              if (isGuestLogIn &&
                ...
                finalAddress = AddressModel(
                  ...
                  address: userSavedAddress?.address ?? '',
                  latitude: userSavedAddress?.latitude,
                  longitude: userSavedAddress?.longitude,
                  zoneId: userSavedAddress?.zoneId,
              ...
              if (widget.storeId == null) {
                ...
                index++) {
                  ...
                  if (Get.find<SplashController>()
                    ...
                    for (int i = 0;
                    i <
                        cart.item!.foodVariations!.length;
                    i++) {
                      if (cart.foodVariations![i]
                          .contains(true)) {
                        variations.add(
                          OrderVariation(
                            name: cart.item!
  ...
  List<DropdownItem<int>> _getDropdownAddressList({
    ...
    if (addressList != null && store != null) {
      for (int index = 0; index < addressList.length; index++) {
        if (addressList[index].zoneIds != null &&
            addressList[index].zoneIds!.contains(store.zoneId)) {
          dropDownAddressList.add(
            DropdownItem<int>(
              value: (userAddress != null ? index + 1 : index),
              child: SizedBox(
                width: context.width > Dimensions.webMaxWidth
                    ? Dimensions.webMaxWidth - 50
                    : context.width - 50,
  ...
  Pivot? _getModuleData({required Store? store}) {
    ...
    if (store != null &&
        userAddress?.zoneData != null &&
        Get.find<SplashController>().module != null) {
      for (ZoneData zData in userAddress!.zoneData!) {
        if (zData.modules == null) continue;
  ...
  bool _checkCODActive({required Store? store}) {
    ...
    if (store != null && userAddress?.zoneData != null) {
      for (ZoneData zData in userAddress!.zoneData!) {
        if (zData.id == store.zoneId) {
          isCashOnDeliveryActive = (zData.cashOnDelivery ?? false) &&
              (Get.find<SplashController>().configModel!.cashOnDelivery ?? false);
        }
      }
    }
      ...
      for (ZoneData zData in userAddress!.zoneData!) {
        if (zData.id == store.zoneId) {
          isDigitalPaymentActive = (zData.digitalPayment ?? false) &&
              (Get.find<SplashController>().configModel!.digitalPayment ?? false);
        }
      }
    }
  ...
  double _calculateOriginalDeliveryCharge({
    ...
    Pivot? moduleData;
    if (store != null && address?.zoneData != null) {
    ...
    if (orderType == 'take_away' ||
        ...
        (configModel?.adminFreeDelivery?.status == true &&
            (configModel?.adminFreeDelivery?.type != null &&
                configModel?.adminFreeDelivery?.type == 'free_delivery_to_all_store')) ||
        (configModel?.adminFreeDelivery?.status == true &&
            (configModel?.adminFreeDelivery?.type != null &&

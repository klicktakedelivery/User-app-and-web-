...
class _BannerViewState extends State<BannerView> {
  ...
  Future<void> _precacheFirstImages(List<String?> banners) async {
    ...
    // نحاول نعمل precache لأول صورتين (أثر كبير على سلاسة أول ظهور)
    final first = banners[0];
    final second = banners.length > 1 ? banners[1] : null;

    // CustomImage غالباً يستخدم NetworkImage داخلياً، لكن ما نقدر نضمن.
    // لذلك نستخدم precacheImage مباشرة بـ NetworkImage (ما يضر حتى لو فشل).
    try {
      ...
      // تجاهل أي خطأ في precache (لا نكسر الشاشة)
    }
  }
  ...
  Widget build(BuildContext context) {
    return GetBuilder<BannerController>(builder: (bannerController) {
      ...
      // لا تعرض شيء إذا القائمة فاضية
      if (bannerList != null && bannerList.isEmpty) {
        return const SizedBox.shrink();
      }

      final double bannerHeight =
      GetPlatform.isDesktop ? 500 : MediaQuery.of(context).size.width * 0.45;
      ...
      return Container(
        ...
        child: bannerList != null
          ...
          children: [
                    ...
                    child: Container(
                      ...
                      child: ClipRRect(
                        ...
                        child: RepaintBoundary(
                          // يمنع إعادة رسم ثقيلة للصور أثناء تحريك السلايدر
                          child: CustomImage(
                            image: imageUrl,
                            fit: BoxFit.cover,
                          ),
            ...
            // ✅ استخدم القائمة الصحيحة حسب isFeatured

...
class LocationController extends GetxController implements GetxService {
  ...
  /// كاش بسيط لنتيجة الـ Zone
  ZoneResponseModel? _cachedZoneResponse;
  String? _cachedLat;
  String? _cachedLng;
  ...
  Future<AddressModel> getCurrentLocation(
    ...
    await getAddressFromGeocode(LatLng(myPosition.latitude, myPosition.longitude));
  ...
  Future<ZoneResponseModel> getZone(
    ...
    if (!forceReload &&
        lat != null &&
        lng != null &&
        _cachedZoneResponse != null &&
        _cachedLat == lat &&
        _cachedLng == lng) {
      final cached = _cachedZoneResponse!;
      _inZone = cached.isSuccess;
      _zoneID = cached.zoneIds.isNotEmpty ? cached.zoneIds[0] : 0;

      if (updateInAddress && cached.isSuccess) {
        final saved = AddressHelper.getUserAddressFromSharedPref();
        if (saved != null) {
          saved.zoneData = cached.zoneData;
          await AddressHelper.saveUserAddressInSharedPref(saved);
        }
      }
    ...
    if (lat != null && lng != null) {
      _cachedZoneResponse = responseModel;
      _cachedLat = lat;
      _cachedLng = lng;
    }
    ...
    if (updateInAddress && responseModel.isSuccess) {
      final saved = AddressHelper.getUserAddressFromSharedPref();
      if (saved != null) {
        saved.zoneData = responseModel.zoneData;
        await AddressHelper.saveUserAddressInSharedPref(saved);
      }
    }
  ...
  Future<void> navigateToLocationScreen(
    ...
    // ✅ حالة الضغط من شاشة الهوم: افتح شاشة تغيير الموقع فوراً بدون أي عمليات ثقيلة
    if (fromHome) {
      if (ResponsiveHelper.isDesktop(Get.context)) {
        showGeneralDialog(
          context: Get.context!,
  ...
  Future<void> setStoreAddressToUserAddress(LatLng storeAddress) async {
    ...
    String addressFromGeocode =
    await getAddressFromGeocode(LatLng(storeAddress.latitude, storeAddress.longitude));
    ZoneResponseModel responseModel = await getZone(
